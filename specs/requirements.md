...
- **外部サービス連携**: 必要に応じて OAuth 2.0 等を用いた外部サービス（MCP サーバー等）との認可連携を行う。
...
- **MCP (Model Context Protocol) 対応**: 標準化されたプロトコル (MCP) に準拠した外部サーバーと接続し、AI にツール（機能）を提供できる。
- **ツール実行**: AI が対話中に必要に応じてツール（Web 検索、計算、DB 操作など）を自律的に呼び出し、その結果を回答に反映する。
- **サーバー管理**: MCP サーバーの接続設定（URL、認証情報）を管理できる。
