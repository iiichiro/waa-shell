...
export function ChatInputArea({
  ...
  return (
    <div
      className={`flex flex-col w-full bg-background/60 backdrop-blur-lg rounded-lg overflow-hidden transition-all shadow-sm border ${
        selectedFiles.length > 0 ? 'rounded-t-none border-t-0' : ''
      }`}
    >
      <input
        type="file"
      ...
      {selectedFiles.length > 0 && (
        <div className="flex gap-2 p-2 overflow-x-auto custom-scrollbar border-b bg-muted/10">
      ...
      <div className={`flex w-full ${isMultiLine ? 'flex-col' : 'items-end'}`}>
        <div className={`flex flex-1 min-w-0 ${isMultiLine ? 'w-full' : 'items-end'}`}>
          ...
          <textarea
            ref={textareaRef}
            rows={1}
            className="flex-1 bg-transparent border-none outline-none py-2 px-3 placeholder:text-muted-foreground/50 text-sm min-w-0 resize-none overflow-y-auto custom-scrollbar focus:ring-0 leading-normal text-foreground"
