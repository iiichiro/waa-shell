    ...
    <div className="absolute bottom-full left-0 mb-4 w-96 glass rounded-2xl shadow-2xl overflow-hidden z-[110] animate-in fade-in zoom-in-95">
      <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between bg-white/5">
        <div>
          ...
          <p className="text-[10px] text-text-secondary">{command.description}</p>
        </div>
        <button
          type="button"
          onClick={onCancel}
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4">
        {command.variables.map((v, index) => (
          <div key={v.name} className="space-y-1.5 text-left">
            <div className="flex items-center justify-between">
              <label htmlFor={`var-${v.name}`} className="text-xs font-semibold text-text-primary">
                {v.label}
              </label>
              {v.description && (
                <span className="text-[10px] text-text-secondary">{v.description}</span>
              )}
            </div>
            <textarea
              id={`var-${v.name}`}
        ...
        <div className="pt-2 flex gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 px-3 py-2 rounded-xl text-sm font-medium text-text-secondary hover:bg-white/5 transition-colors"
          >
            キャンセル
          </button>
          <button
            type="submit"
            className="flex-1 px-3 py-2 bg-brand-primary text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-brand-primary/90 transition-all shadow-lg shadow-brand-primary/20"
          >
