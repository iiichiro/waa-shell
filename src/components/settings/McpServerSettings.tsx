        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {servers.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground bg-muted/20 rounded-lg border border-dashed border-border">
              ...
              <p className="text-sm">設定されている MCP サーバはありません</p>
            </div>
          )}
        </div>
      </section>
      ...
      {editingServer &&
        createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="w-full max-w-md bg-background border rounded-lg shadow-xl flex flex-col animate-in zoom-in-95 duration-200">
              <header className="flex items-center justify-between p-4 border-b bg-muted/20 rounded-t-lg">
                <h3 className="font-bold text-foreground flex items-center gap-2">
                  <Pencil className="w-4 h-4 text-primary" />
                  {editingServer.id ? 'サーバ設定を編集' : '新しいサーバを追加'}
                </h3>
                <button
                  type="button"
              ...
              <footer className="p-4 flex justify-end gap-3 border-t bg-muted/10 rounded-b-lg">
                <button
                  type="button"
                  disabled={!!pingingId}
                  onClick={() => handlePing(editingServer)}
...
function McpToolManagerModal({ server, onClose }: { server: McpServer; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="w-full max-w-lg bg-background border rounded-lg shadow-xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
        <header className="flex items-center justify-between p-4 border-b bg-muted/20 rounded-t-lg">
          ...
          {!server.isActive ? (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                <p className="text-sm font-bold">サーバが非アクティブです</p>
                <p className="text-xs text-muted-foreground">
                  ツールの一覧を取得するには、サーバを有効にする必要があります。
                </p>
              </div>
            </div>
          ) : isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground bg-muted/20 rounded-lg border border-dashed border-border">
              <Wrench className="w-8 h-8 mb-2 mx-auto opacity-20" />
              <p className="text-sm">利用可能なツールはありません</p>
            </div>
          ) : (
            tools.map((tool) => {
              ...
              return (
                <div
                  key={tool.name}
                  className="flex items-start justify-between p-4 rounded-lg border bg-muted/30 text-foreground transition-all hover:border-primary/20 group"
                >
                  ...
                  <button
                    ...
                    onClick={() => setToolEnabled(tool.name, !isEnabled)}
                    className={`w-11 h-6 rounded-full border transition-colors relative shrink-0 mt-1 ${
                      isEnabled
                        ? 'bg-primary border-primary'
                  ...
                  >
                    <span
                      className={`block w-4 h-4 rounded-full shadow-sm transition-transform absolute top-1 ${
                        isEnabled ? 'left-6 bg-primary-foreground' : 'left-1 bg-background'
                      }`}
                    />
                  </button>
                </div>
